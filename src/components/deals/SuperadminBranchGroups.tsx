...
export default function SuperadminBranchGroups({ branchSlug }: { branchSlug: string }) {
  const { deals, dealTransactions, branches } = useApp();
  const branchId = branches.find(b => b.slug === branchSlug)?.id;
  const branchDeals = deals.filter(d => d.managingBranchId === branchId);
  const basePath = `/group/${branchSlug}`;
  ...
  const totalGroups = new Set(branchDeals.map(d => d.groupName || 'General')).size;

  const processedDeals = useMemo(() => {
    return branchDeals.map((deal) => {
  ...
  }, [branchDeals, filteredTransactions]);

  const totalDeals = processedDeals.reduce((acc, d) => acc + d.totalDeals, 0);
  const totalDealAmount = branchDeals.reduce((acc, deal) => {
  ...
  const totalExpense = branchDeals.reduce((acc, deal) => {
    const groupDeals = filteredTransactions.filter(t => t.dealId === deal.id);
  ...
  const totalGoldGrams = branchDeals.reduce((acc, deal) => {
    const unsettledDeals = filteredTransactions.filter(t => t.dealId === deal.id && t.fixOrUnfix === 'unfixed');
  ...
  return (
    <>
      ...
      <CreateDealModal open={showCreate} onClose={() => setShowCreate(false)} isBranchView={true} branches={branches.filter(b => b.id === branchId)} />
    </>
  );
}
