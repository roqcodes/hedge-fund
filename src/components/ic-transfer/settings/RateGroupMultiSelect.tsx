...
type Props = {
  ...
  placeholder?: string;
  className?: string;
  /** Hide chips above the trigger so the control stays one row tall (bulk toolbar). */
  compact?: boolean;
};

export default function RateGroupMultiSelect({
  groups,
  selectedIds,
  onChange,
  label,
  ...
  const selectedGroups = useMemo(
    () => selectedIds.map(id => groups.find(g => g.id === id)).filter(Boolean) as ICRateGroup[],
    [selectedIds, groups],
  );
  ...
  return (
    <div ref={wrapperRef} className={`relative min-w-0 ${className}`}>
      ...
      {!compact && selectedGroups.length > 0 ? (
      ...
      <div
        className={`${formInput} flex !cursor-pointer select-none items-center justify-between !py-3 !pr-8 text-sm bg-white sm:!py-3.5`}
        onClick={() => {
          setIsOpen(prev => !prev);
          if (!isOpen) setSearch('');
        }}
      >
        <span className={`truncate ${selectedGroups.length === 0 ? 'text-slate-400' : 'text-slate-900'}`}>
          {triggerLabel}
        </span>
        <svg
          width="16"
      ...
      {isOpen ? (
        <div className="absolute z-50 mt-1 flex max-h-72 w-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg animate-[fade-in-up_0.15s_ease-out_both]">
          ...
          <div className="flex-1 overflow-y-auto p-1 scrollbar-thin">
            {filteredGroups.length === 0 ? (
              <div className="px-3 py-2 text-center text-sm text-slate-500">No groups found</div>
            ) : (
