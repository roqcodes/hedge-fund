...
const CURRENCIES = ['AED', 'IDR', 'USDT'] as const;
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Record Payment" maxWidth="max-w-lg w-[95vw]">
      <div className="space-y-5 pb-4">
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map(c => (
                  ...
                  >
                    ...
                    <span className="ml-auto text-[10px] font-bold uppercase tracking-wider text-slate-400">{c.currency || 'AED'}</span>
                  </button>
                ))
              )}
            </div>
        ...
        {/* Settlement currency */}
        <div>
          <label className={labelClass}>Settle in</label>
            ...
            {CURRENCIES.map(curr => (
              ...
              >
                <p className={`text-xs font-extrabold uppercase tracking-wider ${
                  settlementCurrency === curr ? 'text-white' : 'text-slate-500'
                }`}>{curr}</p>
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={labelClass}>Amount ({settlementCurrency})</label>
            <input
              type="number"
              step="0.01"
              min="0"
        ...
        {/* Summary */}
        {customerId && numAmount > 0 && rateValid && (
          <div className={`rounded-xl border px-4 py-3 ${
            ...
            <p className={`text-sm font-bold mt-0.5 ${direction === 'credit' ? 'text-emerald-800' : 'text-red-800'}`}>
              {direction === 'credit'
                ? `${selectedCustomer?.name} pays ${numAmount.toFixed(2)} ${settlementCurrency}`
                : `Branch pays ${numAmount.toFixed(2)} ${settlementCurrency} to ${selectedCustomer?.name}`}
            </p>
            {needsRate && numRate > 0 && (
