...
type SortField = 'Name' | 'Account ID' | 'Email';
            ...
            <table className={`${dataTable} min-w-[720px]`}>
              <thead>
                <tr>
                  ...
                  <SortTh field="Name" />
                  <th className="px-3 pb-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-400 sm:px-5">Phone</th>
                  <SortTh field="Email" />
                  {canManageAgents && (
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  <SkeletonRows cols={canManageAgents ? 6 : 5} />
                ) : sortedAgents.length === 0 ? (
                  <tr>
                    <td colSpan={canManageAgents ? 6 : 5} className="px-6 py-10 text-center text-sm text-slate-400">
                      ...
                      {canManageAgents && (
                        <td className="border-y border-r border-black/5 bg-white px-3 py-3.5 text-right last:rounded-r-2xl sm:px-5 sm:py-4">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => setEditingAgent(agent)} className="text-slate-400 hover:text-accent transition-colors p-1 rounded-lg hover:bg-accent/5" title="Edit Agent">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
                                <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                              </svg>
                            </button>
                            <button onClick={() => setDeletingAgent(agent)} className="text-slate-400 hover:text-red-500 transition-colors p-1 rounded-lg hover:bg-red-50" title="Delete Agent">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
