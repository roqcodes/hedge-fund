...
export default function SupplierManagementPage() {
  ...
  const handleSaveSupplier = async (data: any) => {
    if (data.id) {
      await updateICSupplier(data.id, data.name, data.phone, data.commission, data.regionId, data.email, data.address);
    } else {
      await addICSupplier(data.name, data.phone, data.commission, data.regionId, data.email, data.address);
    }
  ...
  return (
    <ICTransferSettingsUsersPage
      ...
      subtitle="Manage supplier settings and rates"
      data={icSuppliers}
      onEditItem={openEdit}
